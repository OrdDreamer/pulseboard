...
Створіть файл `.env` в кореневій директорії проєкту (поруч з `manage.py`):

```bash
cp .env.example .env
```

Відредагуйте файл `.env` та встановіть ваш `SECRET_KEY`. Згенеруйте секретний ключ:

```bash
python -c 'from django.core.management.utils import get_random_secret_key; print(get_random_secret_key())'
```

Скопіюйте згенерований ключ та вставте його в файл `.env` як значення для `SECRET_KEY`.

**Важливо:** 
- Файл `.env` вже є в `.gitignore` і не буде закомічений до репозиторію
- Ніколи не комітьте ваш фактичний `.env` файл з реальними секретами
- Завжди використовуйте сильний, унікальний `SECRET_KEY` в production
